...
FILE_PATH={
    "user"      : "src/demo_20/user/",
    "playlist"  : "src/demo_20/playlist/",
    "song"      : "src/demo_20/song/"
}
...
def parse_playlistPage(page_source,id=0,recursive=False,recur_limit=sys.maxsize):
    soup = BeautifulSoup(page_source, 'lxml')
    ...
    # 字典存储结果
    result_dict = {"meta" : {"count" : 0, "start" : 1, "end":1}, "creator": {}, "playlist":{}}
...
def parse_userPage(page_source,id=0,recursive=False,recur_limit=sys.maxsize):
...
def demo():
    ...
    recur_limit_song   = sys.maxsize  # 一个用户最多拉多少个他的歌单 (0-sys.maxsize)
    recur_limit_lyrics = 0          # 一个歌单最多拉多少个的歌词  （0-sys.maxsize)
